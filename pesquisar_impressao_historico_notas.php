...
$pai = $Linha['pai'];
//
$Consulta2 = mysqli_query($Conexao, "SELECT bimestre_media.*,disciplinas.disciplina FROM `bimestre_media`,`disciplinas` WHERE `id_bimestre_media_aluno` = '$id_aluno' AND disciplinas.id = `id_bimestre_media_disciplina` AND excluido = 'N' ORDER BY ficha_descritiva  ");
//      
...
<html lang="pt-br">
    <head>
        <meta charset="utf-8">
        <title>IMPRESSÃO DE HISTÓRICO</title>
    ...
    <body>
        ...
        $Consulta21 = mysqli_query($Conexao, "SELECT bimestre_media.*,disciplinas.disciplina FROM `bimestre_media`,`disciplinas` WHERE `id_bimestre_media_aluno` = '$id_aluno' GROUP BY ano ORDER BY `bimestre_media`.`ano` ASC ");
        $Linhas = mysqli_num_rows($Consulta21);
        while ($linhaConsulta21 = mysqli_fetch_array($Consulta21, MYSQLI_BOTH)) {
            $ano = $linhaConsulta21['ano'];
            $ano_conv = "";
            if (strlen($ano) !== 4) {
                $ano_conv = substr($ano, 0, -2);
            } else {
                $ano_conv = $ano;
            }
            $cont++;
            //
            //Alimentação das tabelas por ano
            ...
            echo "</tr>";
            echo "<tr>";
            echo "<td>Ano Letivo:&nbsp;&nbsp;$ano_conv </td>";
            echo "</tr>";
            echo "<tr>";
            ...
            echo "</div>";
            echo "<div>";
            echo "<p><input type='text' name = 'inputObs'  value= '$inputObs2' maxlength = '53' class = 'inputObs' ></p>";
            echo "</div>";
            echo "<div>";
            ...
            echo "<thead>";
            echo "<tr>";
            echo "<th colspan = '3' style = 'height:0.9cm'> ";
            echo "</th>";
            echo "</tr>";
            echo "</thead>";
            echo "<tbody>";
            ...
            . "&nbsp;&nbsp;<input type='checkbox' $teste_aprov/>"
            . "</td>";
            echo "<td> Reprovado"
            . "&nbsp;&nbsp;<input type='checkbox' $teste_repr/>"
            . "</td>";
            echo "</tr>";
            //
            //
