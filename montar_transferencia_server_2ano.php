...
//
$Consulta2 = mysqli_query($Conexao, "SELECT bimestre_media.*,disciplinas.disciplina FROM `bimestre_media`,`disciplinas` WHERE `id_bimestre_media_aluno` = '$id_aluno' AND `ano` = '$ano1' AND disciplinas.id = `id_bimestre_media_disciplina` AND `bnc` = 'N' ORDER BY ficha_descritiva ");
while ($linhaConsulta2 = mysqli_fetch_array($Consulta2, MYSQLI_BOTH)) {
    $nota = $linhaConsulta2['nota'];
    $frequencia = $linhaConsulta2['frequencia'];
    $escola_horas = $linhaConsulta2['aluno'];
