...
if ($Linhas > 0) {
    while ($Registro = mysqli_fetch_array($Consultas)) {
        ...
        $procura = array_search("$material33", $materiais);
        //
        if ($procura) {
            //
            ...
            $qdt_material3 = $nome3 = $Registro['nome'];
            //
            if ($procura == "1") {
            ...
            $Consulta = mysqli_query($Conexao, "SELECT * FROM `materiais` WHERE id = '$material33'");
            $Registro = mysqli_fetch_array($Consulta, MYSQLI_BOTH);
            $quantidade_arq = $Registro['quantidade'];
            $qdt_material3 = $nome3 = $Registro['nome'];
        }
...
//echo "valor pedido $quantidade33" . "<br>";

$msg3 = "";
if ($quantidade33 > $html) {
    $quantidade33 = $html;
   // echo "possivel = $quantidade33" . "<br>";
    $msg3 = "O Item $nome3 não Tinha o Suficiente,";
}
//echo "final $quantidade33" . "<br>";
